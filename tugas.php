<?php
    ...
    if ($selected_matkul) {
        $sql_tugas .= " WHERE matkul_id = '".mysqli_real_escape_string($conn, $selected_matkul)."'";
    }
    $sql_tugas .= " ORDER BY uploaded_at ASC";
...
<div class="sm:ml-64 mt-14">
    <div class="p-4">
        <div class="pb-4 text-2xl font-bold text-left rtl:text-right text-gray-900 bg-white dark:text-white dark:bg-gray-800">
            Tugas Kelas 1 D4 Teknik Informatika B
            <p class="mt-1 text-sm font-normal text-gray-500 dark:text-gray-400">This page displays all the assignments for the class. You can view, create, and manage assignments here.</p>
            </p>
        </div>
        
        <div class="flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4 bg-white dark:bg-gray-900">
            <?php if (isset($_SESSION['role']) && in_array($_SESSION['role'], ['lecturer', 'admin'])): ?>
                <div><?php include 'tugas-create.php'; ?></div>
            <?php endif; ?>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            <?php
                ...
                <div class="bg-white p-3 border border-gray-200 rounded-lg shadow-sm w-full dark:bg-gray-800 dark:border-gray-700">
                    <div class="flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4 bg-white dark:bg-gray-900">
                        <h5 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white"><?php echo $row_tugas["name"]; ?></h5>
                        ...
                        <div id="<?php echo $uniqueId; ?>" class="z-10 hidden text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44 dark:bg-gray-700">
                            <ul class="py-2" aria-labelledby="dropdownButton">
                                <li>
                                    <a href="process/materi-remove.php?id=<?php echo $row_tugas["id"]; ?>" class="block px-4 py-2 text-sm text-red-600 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Delete</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                    
                    <p class="mb-3 font-normal text-gray-500 dark:text-gray-400"><?php echo $row_tugas["content"]; ?></p>
                    <p class="mb-3 font-normal italic text-gray-500 dark:text-gray-400">Diupload pada tanggal <?php echo date("d M Y, H:i", strtotime($row_tugas["uploaded_at"])); ?></p>
                    <div class="text-center px-5 py-2.5 bg-sky-600 hover:bg-sky-700 focus:ring-4 focus:outline-none focus:ring-sky-300 rounded-lg dark:bg-sky-600">
                        <a href="tugas-view.php?id=<?php echo $row_tugas['id']; ?>" class="inline-flex font-medium items-center text-white">
                            See more
