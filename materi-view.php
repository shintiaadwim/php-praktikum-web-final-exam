<?php
    ...
    $folder = "files/"; // Folder tempat file materi disimpan
    $file = null;       // Inisialisasi variabel file

    // Jika ada parameter id, ambil nama file dari database
    if (isset($_GET['id'])) {
        $id = intval($_GET['id']);
        $query = mysqli_query($conn, "SELECT path FROM materi WHERE id = $id");
...
<div class="sm:ml-64 mt-14">
    <div class="p-4">
        ...
        <?php if (isset($_SESSION['role']) && in_array($_SESSION['role'], ['lecturer', 'mahasiswa'])): ?>
            <!-- Menampilkan file pada inframe -->
            <?php if ($file && file_exists($filePath)) { ?> 
                <!-- #toolbar=1 : untuk menampilkan navigasi -->
